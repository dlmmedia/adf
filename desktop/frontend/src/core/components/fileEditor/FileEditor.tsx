...
interface FileEditorProps {
  ...
  onMergeFiles?: (files: GenTechFile[]) => void;
  toolMode?: boolean;
...
const FileEditor = ({
  ...
  const activeGenTechFileStubs = useMemo(() => selectors.getGenTechFileStubs(), [state.files.byId, state.files.ids]);
  ...
  // Use activeGenTechFileStubs directly - no conversion needed
    ...
    try {
      if (uploadedFiles.length > 0) {
        ...
        if (Number.isFinite(maxAllowed)) {
          const nowSelectedIds = selectors.getSelectedGenTechFileStubs().map(r => r.id);
  ...
  const toggleFile = useCallback((fileId: FileId) => {
    ...
    const targetRecord = activeGenTechFileStubs.find(r => r.id === fileId);
  ...
  }, [setSelectedFiles, toolMode, _setStatus, activeGenTechFileStubs, selectedTool?.maxFiles]);
  ...
  const handleReorderFiles = useCallback((sourceFileId: FileId, targetFileId: FileId, selectedFileIds: FileId[]) => {
    const currentIds = activeGenTechFileStubs.map(r => r.id);
  ...
  }, [activeGenTechFileStubs, reorderFiles, _setStatus]);



  // File operations using context
  const handleCloseFile = useCallback((fileId: FileId) => {
    const record = activeGenTechFileStubs.find(r => r.id === fileId);
  ...
  }, [activeGenTechFileStubs, selectors, removeFiles, setSelectedFiles, selectedFileIds]);
  ...
  const handleDownloadFile = useCallback(async (fileId: FileId) => {
    ...
    if (record && file) {
      ...
      if (result.savedPath) {
        ...
        fileActions.updateGenTechFileStub(fileId, {
  ...
  }, [activeGenTechFileStubs, selectors, fileActions]);
    ...
    if (record && file) {
      try {
        ...
        if (result.success && result.extractedStubs.length > 0) {
          ...
          await fileActions.addGenTechFileStubs(result.extractedStubs);
  ...
  }, [activeGenTechFileStubs, selectors, fileActions, removeFiles]);
  ...
  }, [activeGenTechFileStubs, setSelectedFiles, navActions.setWorkbench]);
  ...
  return (
    ...
    >
      <Box pos="relative" style={{ overflow: 'auto' }}>
        ...
        {activeGenTechFileStubs.length === 0 ? (
          <Center h="60vh">
          ...
          >
            ...
            {activeGenTechFileStubs.map((record, index) => {
              return (
                <FileEditorThumbnail
                  key={record.id}
                  file={record}
                  index={index}
                  totalFiles={activeGenTechFileStubs.length}
