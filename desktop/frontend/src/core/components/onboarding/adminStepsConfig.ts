...
export function createAdminStepsConfig({ t, actions }: CreateAdminStepsConfigArgs): Record<AdminTourStep, StepType> {
  ...
  return {
    ...
    [AdminTourStep.WRAP_UP]: {
      ...
      content: t('adminOnboarding.wrapUp', "That's the admin tour! You've seen the enterprise features that make DLM ADF a powerful, customisable solution for organisations. Access this tour anytime from the <strong>Help</strong> menu."),
      position: 'right',
      padding: 10,
      action: () => {
        removeAllGlows();
