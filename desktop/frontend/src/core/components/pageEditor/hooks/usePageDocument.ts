...
export function usePageDocument(): PageDocumentHook {
  ...
  const activeFileIds = useMemo(() => {
    return allFileIds.filter(id => {
      const stub = selectors.getGenTechFileStub(id);
  ...
  const primaryGenTechFileStub = primaryFileId ? selectors.getGenTechFileStub(primaryFileId) : null;
  const processedFilePages = primaryGenTechFileStub?.processedFile?.pages;
  const processedFileTotalPages = primaryGenTechFileStub?.processedFile?.totalPages;
  ...
  useEffect(() => {
    ...
    if (primaryGenTechFileStub?.processedFile) {
      placeholderDocumentRef.current = null;
      setPlaceholderVersion(v => v + 1);
      return;
    }
      ...
      try {
        ...
        if (!canceled) {
          placeholderDocumentRef.current = {
            ...
            name: selectors.getGenTechFileStub(primaryFileId)?.name ?? file.name,
            file,
            pages,
            totalPages,
          };
  ...
  }, [primaryFileId, primaryGenTechFileStub?.processedFile, selectors]);
  ...
  if (!primaryGenTechFileStub?.processedFile && placeholderDocumentRef.current) {
  ...
  const primaryFile = primaryFileId ? selectors.getFile(primaryFileId) : null;
    ...
    const name =
      namingFileIds.length <= 1
        ? (namingFileIds[0]
            ? selectors.getGenTechFileStub(namingFileIds[0])?.name ?? 'document.pdf'
            : 'document.pdf')
        : namingFileIds
            .map(id => (selectors.getGenTechFileStub(id)?.name ?? 'file').replace(/\.pdf$/i, ''))
            .join(' + ');
    ...
    const createPagesFromFile = (fileId: FileId, startPageNumber: number, isSelected: boolean): PDFPage[] => {
      const adfFileStub = selectors.getGenTechFileStub(fileId);
      if (!adfFileStub) {
        return [];
      }
      ...
      const processedFile = adfFileStub.processedFile;
