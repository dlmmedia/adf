...
export const usePageEditorExport = ({
  ...
  const applyChanges = useCallback(async () => {
    ...
    try {
      ...
      const newGenTechFiles = await actions.addFiles(renamedFiles, {
        selectFiles: true,
      });
      if (newGenTechFiles.length > 0) {
        actions.setSelectedFiles(newGenTechFiles.map((file) => file.fileId));
      }

      if (sourceFileIds.length === 1 && newGenTechFiles.length === 1) {
        const sourceStub = selectors.getGenTechFileStub(sourceFileIds[0]);
        if (sourceStub?.localFilePath) {
          actions.updateGenTechFileStub(newGenTechFiles[0].fileId, {
