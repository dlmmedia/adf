...
interface FileGridProps {
  files: Array<{ file: File; record?: GenTechFileStub }>;
  onRemove?: (index: number) => void;
  onDoubleClick?: (item: { file: File; record?: GenTechFileStub }) => void;
  onView?: (item: { file: File; record?: GenTechFileStub }) => void;
  onEdit?: (item: { file: File; record?: GenTechFileStub }) => void;
    ...
    <Box >
      ...
      >
        {displayFiles
          .filter(item => {
            if (!item.record?.id) {
              console.error('FileGrid: File missing GenTechFileStub with proper ID:', item.file.name);
              return false;
            }
            return true;
          })
