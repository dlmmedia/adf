...
import { isGenTechFile } from '@app/types/fileContext';
...
export default function RightRail() {
  ...
  const handleExportAll = useCallback(async () => {
    if (currentView === 'viewer') {
      ...
      const stub = isGenTechFile(fileToExport) ? selectors.getGenTechFileStub(fileToExport.fileId) : undefined;
      try {
        ...
        if (!result.cancelled && stub && result.savedPath) {
          fileActions.updateGenTechFileStub(stub.id, {
    ...
    if (filesToExport.length > 0) {
      for (const file of filesToExport) {
        const stub = isGenTechFile(file) ? selectors.getGenTechFileStub(file.fileId) : undefined;
        try {
          const result = await downloadFile({
            data: file,
            filename: file.name,
