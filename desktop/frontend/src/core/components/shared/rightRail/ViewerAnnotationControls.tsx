...
export default function ViewerAnnotationControls({ currentView, disabled = false }: ViewerAnnotationControlsProps) {
  ...
  const saveAnnotationsIfNeeded = async () => {
    ...
    try {
      ...
      const parentStub = selectors.getGenTechFileStub(state.files.ids[0]);
      if (!parentStub) return;

      const { adfFiles, stubs } = await createGenTechFilesAndStubs([file], parentStub, 'redact');
      await fileActions.consumeFiles([state.files.ids[0]], adfFiles, stubs);
