...
import type { GenTechFile, GenTechFileStub } from '@app/types/fileContext';
...
export const getFileFromSelection = (
  explicit: GenTechFile | null | undefined,
  fileId: FileId | null,
  selectors: { getFile: (id: FileId) => GenTechFile | undefined | null }
): GenTechFile | null => {
  if (explicit) return explicit;
  if (!fileId) return null;
  return (selectors.getFile(fileId) as GenTechFile | undefined | null) ?? null;
};

export const getStubFromSelection = (
  fileId: FileId | null,
  selectors: { getGenTechFileStub: (id: FileId) => GenTechFileStub | undefined }
): GenTechFileStub | null => {
  if (!fileId) return null;
  const stub = selectors.getGenTechFileStub(fileId);
  return stub ?? null;
};
