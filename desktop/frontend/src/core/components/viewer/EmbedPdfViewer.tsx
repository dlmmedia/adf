...
import { createGenTechFilesAndStubs } from '@app/services/fileStubHelpers';
import NavigationWarningModal from '@app/components/shared/NavigationWarningModal';
import { isGenTechFile, getFormFillFileId } from '@app/types/fileContext';
...
const EmbedPdfViewerContent = ({
  ...
  const bookmarkCacheKey = React.useMemo(() => {
    if (currentFile && isGenTechFile(currentFile)) {
  ...
  const applyChanges = useCallback(async () => {
    ...
    try {
      ...
      // Step 3: Create GenTechFiles and stubs for version history
      ...
      const parentStub = selectors.getGenTechFileStub(currentFileId);
      if (!parentStub) throw new Error('Parent stub not found');

      const { adfFiles, stubs } = await createGenTechFilesAndStubs([file], parentStub, 'multiTool');
      ...
      await actions.consumeFiles([currentFileId], adfFiles, stubs);
  ...
  const handleFormApply = useCallback(async (filledBlob: Blob) => {
    ...
    try {
      ...
      // Create GenTechFiles and stubs for version history
      const { adfFiles, stubs } = await createGenTechFilesAndStubs([file], parentStub, 'multiTool');
          ...
          <Box
            ...
            <LocalEmbedPDF
              ...
              fileName={
                ...
                (currentFile && isGenTechFile(currentFile) ? currentFile.name :
