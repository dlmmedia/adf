...
const ADMIN_TOUR_ORCHESTRATION_FALLBACK: AdminTourOrchestrationContextType = {
  saveAdminState: noopAdmin,
  restoreAdminState: noopAdmin,
  openConfigModal: noopAdmin,
  closeConfigModal: noopAdmin,
  navigateToSection: noopAdmin,
  scrollNavToSection: noopAdmin,
  scrollToSetting: noopAdmin,
};
...
export const useAdminTourOrchestration = (): AdminTourOrchestrationContextType => {
  ...
  if (!context) {
    console.warn('useAdminTourOrchestration: AdminTourOrchestrationProvider not found, using no-op fallback');
    return ADMIN_TOUR_ORCHESTRATION_FALLBACK;
  }
  return context;
};
