/**
 * FileContext - Manages PDF files for DLM ADF multi-tool workflow
 *
...
import { addFiles, addGenTechFileStubs, consumeFiles, undoConsumeFiles, createFileActions, createChildStub, generateProcessedFileMetadata } from '@app/contexts/file/fileActions';
...
function FileContextInner({
  ...
  const selectFiles = (adfFiles: GenTechFile[]) => {
    const currentSelection = stateRef.current.ui.selectedFileIds;
    const newFileIds = adfFiles.map(adfFile => adfFile.fileId);
    dispatch({ type: 'SET_SELECTED_FILES', payload: { fileIds: [...currentSelection, ...newFileIds] } });
  };

  // File operations using unified addFiles helper with persistence
  const addRawFiles = useCallback(async (files: File[], options?: { insertAfterPageId?: string; selectFiles?: boolean; skipAutoUnzip?: boolean }): Promise<GenTechFile[]> => {
    const adfFiles = await addFiles(
      {
        files,
        ...options,
    ...
    if (options?.selectFiles && adfFiles.length > 0) {
      selectFiles(adfFiles);
    }

    return adfFiles;
  }, [enablePersistence, requestConfirmation]);

  const addGenTechFileStubsAction = useCallback(async (adfFileStubs: GenTechFileStub[], options?: { insertAfterPageId?: string; selectFiles?: boolean }): Promise<GenTechFile[]> => {
    // GenTechFileStubs preserve all metadata - perfect for FileManager use case!
    const result = await addGenTechFileStubs(adfFileStubs, options, stateRef, filesRef, dispatch, lifecycleManager);
  ...
  const consumeFilesWrapper = useCallback(async (inputFileIds: FileId[], outputGenTechFiles: GenTechFile[], outputGenTechFileStubs: GenTechFileStub[]): Promise<FileId[]> => {
    return consumeFiles(inputFileIds, outputGenTechFiles, outputGenTechFileStubs, filesRef, dispatch);
  }, []);
  ...
  const runAutomaticPasswordRemoval = useCallback(async (fileId: FileId, password: string): Promise<void> => {
    ...
    const adfUnlockedFile = createGenTechFile(unlockedFile, childStub.id);

    await consumeFilesWrapper([fileId], [adfUnlockedFile], [childStub]);
  ...
  const handleUnlockSubmit = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = await handlePasswordError(
        ...
        t('removePassword.error.failed', 'An error occurred while removing the password from the ADF.')
      );
  ...
  const undoConsumeFilesWrapper = useCallback(async (inputFiles: File[], inputGenTechFileStubs: GenTechFileStub[], outputFileIds: FileId[]): Promise<void> => {
    return undoConsumeFiles(inputFiles, inputGenTechFileStubs, outputFileIds, filesRef, dispatch, indexedDB);
  }, [indexedDB]);

  // File pinning functions - use GenTechFile directly
  const pinFileWrapper = useCallback((file: GenTechFile) => {
    baseActions.pinFile(file.fileId);
  }, [baseActions]);

  const unpinFileWrapper = useCallback((file: GenTechFile) => {
  ...
  const actions = useMemo<FileContextActions>(() => ({
    ...
    addGenTechFileStubs: addGenTechFileStubsAction,
    ...
    updateGenTechFileStub: (fileId: FileId, updates: Partial<GenTechFileStub>) =>
