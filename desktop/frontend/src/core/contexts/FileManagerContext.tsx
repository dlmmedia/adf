...
interface FileManagerContextValue {
  ...
  searchTerm: string;
  selectedFiles: GenTechFileStub[];
  filteredFiles: GenTechFileStub[];
  fileInputRef: React.RefObject<HTMLInputElement | null>;
  selectedFilesSet: Set<FileId>;
  expandedFileIds: Set<FileId>;
  fileGroups: Map<FileId, GenTechFileStub[]>;
  loadedHistoryFiles: Map<FileId, GenTechFileStub[]>;
  isLoading: boolean;
  ...
  onFileSelect: (file: GenTechFileStub, index: number, shiftKey?: boolean) => void;
  onFileRemove: (index: number) => void;
  onHistoryFileRemove: (file: GenTechFileStub) => void;
  onFileDoubleClick: (file: GenTechFileStub) => void;
  onOpenFiles: () => void;
  ...
  onDownloadSingle: (file: GenTechFileStub) => void;
  onToggleExpansion: (fileId: FileId) => void;
  onAddToRecents: (file: GenTechFileStub) => void;
  onUnzipFile: (file: GenTechFileStub) => Promise<void>;
  onNewFilesSelect: (files: File[]) => void;
  onGoogleDriveSelect: (files: File[]) => void;

  // External props
  recentFiles: GenTechFileStub[];
...
interface FileManagerProviderProps {
  ...
  recentFiles: GenTechFileStub[];
  onRecentFilesSelected: (files: GenTechFileStub[]) => void; // For selecting stored files
...
export const FileManagerProvider: React.FC<FileManagerProviderProps> = ({
  ...
  const [loadedHistoryFiles, setLoadedHistoryFiles] = useState<Map<FileId, GenTechFileStub[]>>(new Map()); // Cache for loaded history
  ...
  const fileGroups = useMemo(() => {
    ...
    // Convert GenTechFileStub to FileRecord-like objects for grouping utility
  ...
  const handleFileSelect = useCallback((file: GenTechFileStub, currentIndex: number, shiftKey?: boolean) => {
  ...
  const getSafeFilesToDelete = useCallback((
    fileIds: FileId[],
    allStoredStubs: GenTechFileStub[]
  ): FileId[] => {
    ...
    for (const leafFileId of fileIds) {
      ...
      if (currentFile.versionNumber && currentFile.versionNumber > 1) {
        ...
        const chainFiles = allStoredStubs.filter((file: GenTechFileStub) =>
    ...
    for (const file of allStoredStubs) {
      ...
      if (file.isLeaf !== false && !fileIds.includes(file.id)) {
        ...
        const preservedChainFiles = allStoredStubs.filter((chainFile: GenTechFileStub) =>
  ...
  const performFileDelete = useCallback(async (fileToRemove: GenTechFileStub, fileIndex: number) => {
    const deletedFileId = fileToRemove.id;

    // Get all stored files to analyze lineages
    const allStoredStubs = await fileStorage.getAllGenTechFileStubs();
    ...
    try {
      for (const fileId of filesToDelete) {
        await fileStorage.deleteGenTechFile(fileId as FileId);
      }
    } catch (error) {
  ...
  const handleHistoryFileRemove = useCallback(async (fileToRemove: GenTechFileStub) => {
    const deletedFileId = fileToRemove.id;
    ...
    // Delete safe files from IndexedDB
    try {
        await fileStorage.deleteGenTechFile(deletedFileId);
    } catch (error) {
      console.error('Failed to delete files from chain:', error);
    }
  ...
  const handleFileDoubleClick = useCallback((file: GenTechFileStub) => {
  ...
  const handleDownloadSingle = useCallback(async (file: GenTechFileStub) => {
    try {
    ...
    if (!isCurrentlyExpanded) {
      ...
      if (currentFileMetadata && (currentFileMetadata.versionNumber || 1) > 1) {
        try {
          ...
          // GenTechFileStubs already have all the data we need - no conversion required!
  ...
  const handleAddToRecents = useCallback(async (file: GenTechFileStub) => {
    try {
  ...
  const handleUnzipFile = useCallback(async (file: GenTechFileStub) => {
    try {
      // Load the full file from storage
      const adfFile = await fileStorage.getGenTechFile(file.id);
      if (!adfFile) {
        return;
      }

      // Extract and store files using shared service method
      const result = await zipFileService.extractAndStoreFilesWithHistory(adfFile, file);
