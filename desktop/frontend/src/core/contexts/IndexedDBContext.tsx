...
import { GenTechFileStub, createGenTechFile, createQuickKey } from '@app/types/fileContext';
...
interface IndexedDBContextValue {
  ...
  saveFile: (file: File, fileId: FileId, existingThumbnail?: string) => Promise<GenTechFileStub>;
  loadFile: (fileId: FileId) => Promise<File | null>;
  loadMetadata: (fileId: FileId) => Promise<GenTechFileStub | null>;
  deleteFile: (fileId: FileId) => Promise<void>;

  // Batch operations
  loadAllMetadata: () => Promise<GenTechFileStub[]>;
  loadLeafMetadata: () => Promise<GenTechFileStub[]>; // Only leaf files for recent files list
...
export function IndexedDBProvider({ children }: IndexedDBProviderProps) {
  ...
  const saveFile = useCallback(async (file: File, fileId: FileId, existingThumbnail?: string): Promise<GenTechFileStub> => {
    ...
    const storedFile = await fileStorage.getGenTechFileStub(fileId);

    // Cache the file object for immediate reuse
    fileCache.current.set(fileId, { file, lastAccessed: Date.now() });
    evictLRUEntries();

    // Return GenTechFileStub from the stored file (no conversion needed)
    if (!storedFile) {
  ...
  const loadFile = useCallback(async (fileId: FileId): Promise<File | null> => {
    ...
    // Load from IndexedDB
    const storedFile = await fileStorage.getGenTechFile(fileId);
    if (!storedFile) return null;

    // GenTechFile is already a File object, no reconstruction needed
  ...
  const loadMetadata = useCallback(async (fileId: FileId): Promise<GenTechFileStub | null> => {
    // Load stub directly from storage service
    return await fileStorage.getGenTechFileStub(fileId);
  }, []);
  ...
  const deleteFile = useCallback(async (fileId: FileId): Promise<void> => {
    ...
    await fileStorage.deleteGenTechFile(fileId);
  }, []);

  const loadLeafMetadata = useCallback(async (): Promise<GenTechFileStub[]> => {
    const metadata = await fileStorage.getLeafGenTechFileStubs(); // Only get leaf files

    // All files are already GenTechFileStub objects, no processing needed
    return metadata;

  }, []);

  const loadAllMetadata = useCallback(async (): Promise<GenTechFileStub[]> => {
    const metadata = await fileStorage.getAllGenTechFileStubs();
  ...
  const deleteMultiple = useCallback(async (fileIds: FileId[]): Promise<void> => {
    ...
    await Promise.all(fileIds.map(id => fileStorage.deleteGenTechFile(id)));
  }, []);
