...
export const TourOrchestrationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const savedFilesRef = useRef<GenTechFile[]>([]);

  // Keep a ref to always have the latest files
  const filesRef = useRef<GenTechFile[]>(files);
  React.useEffect(() => {
  ...
  const restoreWorkbenchState = useCallback(async () => {
    ...
    if (currentFiles.length > 0) {
      try {
        await Promise.all(currentFiles.map(file => fileStorage.deleteGenTechFile(file.fileId)));
    ...
    if (savedFilesRef.current.length > 0) {
      // Create fresh File objects from GenTechFile to avoid ID conflicts
  ...
  const loadSampleFile = useCallback(async () => {
    try {
      ...
      closeFilesModal();
      const response = await fetch(`${BASE_PATH}/samples/DLM-ADF.pdf`);
      const blob = await response.blob();
      const file = new File([blob], 'DLM-ADF.pdf', { type: 'application/pdf' });
...
const TOUR_ORCHESTRATION_FALLBACK: TourOrchestrationContextType = {
  saveWorkbenchState: noop,
  restoreWorkbenchState: noopAsync,
  backToAllTools: noop,
  selectCropTool: noop,
  loadSampleFile: noopAsync,
  switchToViewer: noop,
  switchToPageEditor: noop,
  switchToActiveFiles: noop,
  selectFirstFile: noop,
  pinFile: noop,
...
export const useTourOrchestration = (): TourOrchestrationContextType => {
  ...
  if (!context) {
    console.warn('useTourOrchestration: TourOrchestrationProvider not found, using no-op fallback');
    return TOUR_ORCHESTRATION_FALLBACK;
  }
  return context;
};
