...
export function fileContextReducer(state: FileContextState, action: FileContextAction): FileContextState {
  switch (action.type) {
    case 'ADD_FILES': {
      const { adfFileStubs } = action.payload;
      const newIds: FileId[] = [];
      const newById: Record<FileId, GenTechFileStub> = { ...state.files.byId };

      adfFileStubs.forEach(record => {
    ...
    case 'CONSUME_FILES': {
      const { inputFileIds, outputGenTechFileStubs } = action.payload;

      return processFileSwap(state, inputFileIds, outputGenTechFileStubs);
    }


    case 'UNDO_CONSUME_FILES': {
      const { inputGenTechFileStubs, outputFileIds } = action.payload;

      return processFileSwap(state, outputFileIds, inputGenTechFileStubs);
    }
