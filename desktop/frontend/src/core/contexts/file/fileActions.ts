...
/**
 * Create a child GenTechFileStub from a parent stub with proper history management.
 * Used when a tool processes an existing file to create a new version with incremented history.
 *
 * @param parentStub - The parent GenTechFileStub to create a child from
 ...
 * @returns New child GenTechFileStub with proper version history
 */
...
export async function addFiles(
  ...
  try {
    const adfFileStubs: GenTechFileStub[] = [];
  ...
  for (const file of filesToProcess) {
    ...
    const fileStub = createNewGenTechFileStub(file, fileId);
    ...
    adfFileStubs.push(fileStub);

    // Dispatch immediately so each file appears as soon as it is processed
    dispatch({ type: 'ADD_FILES', payload: { adfFileStubs: [fileStub] } });

    // Create GenTechFile directly
    const adfFile = createGenTechFile(file, fileId);
    ...
    scheduleMetadataHydration(async () => {
      ...
      const updates: Partial<GenTechFileStub> = {};
      ...
      if (Object.keys(updates).length > 0) {
        lifecycleManager.updateGenTechFileStub(fileId, updates, stateRef);
      }
    });
  }

  // Persist to storage if enabled using fileStorage service
  if (enablePersistence && adfFiles.length > 0) {
    await Promise.all(adfFiles.map(async (adfFile, index) => {
      try {
        // Get corresponding stub with all metadata
        const fileStub = adfFileStubs[index];

        // Store using the cleaner signature - pass GenTechFile + GenTechFileStub directly
        await fileStorage.storeGenTechFile(adfFile, fileStub);

        if (DEBUG) console.log(`📄 addFiles: Stored file ${adfFile.name} with metadata:`, fileStub);
      } catch (error) {
        console.error('Failed to persist file to storage:', adfFile.name, error);
      }
    }));
  }
 ...
 * Now accepts pre-created GenTechFiles and GenTechFileStubs to preserve all metadata
 */
export async function consumeFiles(
  inputFileIds: FileId[],
  outputGenTechFiles: GenTechFile[],
  outputGenTechFileStubs: GenTechFileStub[],
  filesRef: React.MutableRefObject<Map<FileId, File>>,
  dispatch: React.Dispatch<FileContextAction>
): Promise<FileId[]> {
  if (DEBUG) console.log(`📄 consumeFiles: Processing ${inputFileIds.length} input files, ${outputGenTechFiles.length} output files with pre-created stubs`);

  // Validate that we have matching files and stubs
  if (outputGenTechFiles.length !== outputGenTechFileStubs.length) {
    throw new Error(`Mismatch between output files (${outputGenTechFiles.length}) and stubs (${outputGenTechFileStubs.length})`);
  }

  // Store GenTechFiles in filesRef using their existing IDs (no ID generation needed)
  for (let i = 0; i < outputGenTechFiles.length; i++) {
    const adfFile = outputGenTechFiles[i];
    const stub = outputGenTechFileStubs[i];

    if (adfFile.fileId !== stub.id) {
      console.warn(`📄 consumeFiles: ID mismatch between GenTechFile (${adfFile.fileId}) and stub (${stub.id})`);
    }

    filesRef.current.set(adfFile.fileId, adfFile);

    if (DEBUG) console.log(`📄 consumeFiles: Stored GenTechFile ${adfFile.name} with ID ${adfFile.fileId}`);
  }

  // Mark input files as processed in storage (no longer leaf nodes)
  if(!outputGenTechFileStubs.reduce((areAllV1, stub) => areAllV1 && (stub.versionNumber == 1), true)) {
    await Promise.all(
  ...
  for (let i = 0; i < outputGenTechFiles.length; i++) {
    ...
    try {
      ...
      await fileStorage.storeGenTechFile(adfFile, stub);

      if (DEBUG) console.log(`📄 Saved GenTechFile ${adfFile.name} directly to storage with complete metadata:`, {
        fileId: adfFile.fileId,
      ...
      console.error('Failed to persist output file to fileStorage:', adfFile.name, error);
    }
  }
  ...
  dispatch({
    ...
    payload: {
      inputFileIds,
      outputGenTechFileStubs: outputGenTechFileStubs
    }
  });

  if (DEBUG) console.log(`📄 consumeFiles: Successfully consumed files - removed ${inputFileIds.length} inputs, added ${outputGenTechFileStubs.length} outputs`);
  // Return the output file IDs for undo tracking
  return outputGenTechFileStubs.map(stub => stub.id);
}
...
async function restoreFilesAndCleanup(
  filesToRestore: Array<{ file: File; record: GenTechFileStub }>,
...
export async function undoConsumeFiles(
  inputFiles: File[],
  inputGenTechFileStubs: GenTechFileStub[],
  ...
  if (DEBUG) console.log(`📄 undoConsumeFiles: Restoring ${inputGenTechFileStubs.length} input files, removing ${outputFileIds.length} output files`);

  // Validate inputs
  if (inputFiles.length !== inputGenTechFileStubs.length) {
    throw new Error(`Mismatch between input files (${inputFiles.length}) and records (${inputGenTechFileStubs.length})`);
  }
  ...
  try {
    ...
    const filesToRestore = inputFiles.map((file, index) => ({
      file,
      record: inputGenTechFileStubs[index]
    }));
    ...
    const stubsWithDirtyMarked = inputGenTechFileStubs.map(stub => {
    ...
    dispatch({
      ...
      payload: {
        inputGenTechFileStubs: stubsWithDirtyMarked,
        outputFileIds
      }
    });

    if (DEBUG) console.log(`📄 undoConsumeFiles: Successfully undone consume operation - restored ${inputGenTechFileStubs.length} inputs, removed ${outputFileIds.length} outputs`);
  } catch (error) {
...
/**
 * Add files using existing GenTechFileStubs from storage - preserves all metadata
 * Use this when loading files that already exist in storage (FileManager, etc.)
 * GenTechFileStubs come with proper thumbnails, history, processing state
 */
export async function addGenTechFileStubs(
  adfFileStubs: GenTechFileStub[],
  ...
  try {
    ...
    for (const stub of adfFileStubs) {
      ...
      if (existingQuickKeys.has(stub.quickKey || '')) {
        if (DEBUG) console.log(`📄 Skipping duplicate GenTechFileStub: ${stub.name}`);
        continue;
      }
      ...
      dispatch({ type: 'ADD_FILES', payload: { adfFileStubs: [record] } });
      ...
      scheduleMetadataHydration(async () => {
        const adfFile = await fileStorage.getGenTechFile(fileId);
        if (!adfFile) {
          return;
        }

        // Store the loaded file in filesRef
        filesRef.current.set(fileId, adfFile);

        // Check if processedFile data needs regeneration
        if (adfFile.type.startsWith('application/pdf')) {
          ...
          if (needsProcessing) {
            // Regenerate metadata
            const processedFileMetadata = await generateProcessedFileMetadata(adfFile);
...
export const createFileActions = (dispatch: React.Dispatch<FileContextAction>) => ({
  ...
  updateGenTechFileStub: (fileId: FileId, updates: Partial<GenTechFileStub>) => dispatch({ type: 'UPDATE_FILE_RECORD', payload: { id: fileId, updates } })
});
