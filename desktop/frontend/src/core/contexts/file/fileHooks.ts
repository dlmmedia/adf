...
import { GenTechFileStub, GenTechFile } from '@app/types/fileContext';
 ...
 */
export function useCurrentFile(): { file?: File; record?: GenTechFileStub } {
  ...
  return useMemo(() => ({
    ...
    record: primaryFileId ? selectors.getGenTechFileStub(primaryFileId) : undefined
...
export function useFileManagement() {
  ...
  return useMemo(() => ({
    ...
    updateGenTechFileStub: actions.updateGenTechFileStub,
 ...
 */
export function useGenTechFileStub(fileId: FileId): { file?: File; record?: GenTechFileStub } {
  const { state, selectors } = useFileState();
  const fileRecord = state.files.byId[fileId];

  return useMemo(() => ({
    file: selectors.getFile(fileId),
    record: selectors.getGenTechFileStub(fileId)
 ...
 */
export function useAllFiles(): { files: GenTechFile[]; fileStubs: GenTechFileStub[]; fileIds: FileId[] } {
  const { state, selectors } = useFileState();

  return useMemo(() => ({
    files: selectors.getFiles(),
    fileStubs: selectors.getGenTechFileStubs(),
 ...
 */
export function useSelectedFiles(): { selectedFiles: GenTechFile[]; selectedFileStubs: GenTechFileStub[]; selectedFileIds: FileId[] } {
  const { state, selectors } = useFileState();

  return useMemo(() => ({
    selectedFiles: selectors.getSelectedFiles(),
    selectedFileStubs: selectors.getSelectedGenTechFileStubs(),
    selectedFileIds: state.ui.selectedFileIds
  }), [state.ui.selectedFileIds, state.files.byId, selectors]);
}
