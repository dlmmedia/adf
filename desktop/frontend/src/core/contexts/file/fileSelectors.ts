...
export function createFileSelectors(
  ...
  return {
    getFile: (id: FileId) => {
      ...
      return file ? createGenTechFile(file, id) : undefined;
    },
    ...
    getFiles: (ids?: FileId[]) => {
      ...
      return currentIds
        ...
        .filter(Boolean) as GenTechFile[];
    },

    getGenTechFileStub: (id: FileId) => stateRef.current.files.byId[id],

    getGenTechFileStubs: (ids?: FileId[]) => {
      const currentIds = ids || stateRef.current.files.ids;
    ...
    getPinnedGenTechFileStubs: () => {
      return Array.from(stateRef.current.pinnedFiles)
        .map(id => stateRef.current.files.byId[id])
        .filter(Boolean);
    },

    isFilePinned: (file: GenTechFile) => {
 ...
 */
export function buildQuickKeySet(adfFileStubs: Record<FileId, GenTechFileStub>): Set<string> {
  const quickKeys = new Set<string>();
  Object.values(adfFileStubs).forEach(record => {
    if (record.quickKey) {
...
export function getPrimaryFile(
  stateRef: React.MutableRefObject<FileContextState>,
  filesRef: React.MutableRefObject<Map<FileId, File>>
): { file?: File; record?: GenTechFileStub } {
