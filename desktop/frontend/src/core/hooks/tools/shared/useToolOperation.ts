...
import { GenTechFile, extractFiles, FileId, GenTechFileStub } from '@app/types/fileContext';
...
import { createNewGenTechFileStub } from '@app/types/fileContext';
...
export const useToolOperation = <TParams>(
  ...
  const executeOperation = useCallback(async (
    ...
    const validFiles: GenTechFile[] = selectedFiles.filter(file => file.size > 0);
    ...
    window.addEventListener(FILE_EVENTS.markError, errorListener as EventListener);
      ...
      if (processedFiles.length > 0) {
        ...
        const { inputFileIds, inputGenTechFileStubs } = buildInputTracking(validFiles, selectors);
        ...
        if (isVersionOp) {
          ...
          const downloadLocalPath =
            selectors.getGenTechFileStub(validFiles[0].fileId)?.localFilePath ?? null;
          ...
          const successInputStubs = successSourceIds
            .map((id) => selectors.getGenTechFileStub(id))
            .filter(Boolean) as GenTechFileStub[];
          ...
          const { outputGenTechFileStubs, outputGenTechFiles } = buildOutputPairs(
            processedFiles, thumbnails, processedFileMetadataArray,
            (file, thumbnail, metadata, index) => createChildStub(
              successInputStubs[index] || inputGenTechFileStubs[index] || inputGenTechFileStubs[0],
          ...
          const outputFileIds = await consumeFiles(toConsumeInputIds, outputGenTechFiles, outputGenTechFileStubs);
          ...
          if (toConsumeInputIds.length === 1 && outputFileIds.length === 1) {
            const inputStub = selectors.getGenTechFileStub(toConsumeInputIds[0]);
            if (inputStub?.localFilePath) {
              fileActions.updateGenTechFileStub(outputFileIds[0], {
          ...
          lastOperationRef.current = {
            ...
            inputGenTechFileStubs: inputGenTechFileStubs.map(record => ({ ...record })),
            outputFileIds
          };
          ...
          const { outputGenTechFileStubs, outputGenTechFiles } = buildOutputPairs(
            processedFiles, thumbnails, processedFileMetadataArray,
            (file, thumbnail, metadata) => createNewGenTechFileStub(file, undefined, thumbnail, metadata)
          );
  ...
  const undoOperation = useCallback(async () => {
    ...
    const { inputFiles, inputGenTechFileStubs, outputFileIds } = lastOperationRef.current;

    // Validate that we have data to undo
    if (inputFiles.length === 0 || inputGenTechFileStubs.length === 0) {
    ...
    try {
      ...
      await undoConsumeFiles(inputFiles, inputGenTechFileStubs, outputFileIds);
