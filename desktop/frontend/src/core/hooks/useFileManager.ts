...
export const useFileManager = () => {
  ...
  const convertToFile = useCallback(async (fileStub: GenTechFileStub): Promise<File> => {
    if (!indexedDB) {
  ...
  const loadRecentFiles = useCallback(async (): Promise<GenTechFileStub[]> => {
    setLoading(true);
    try {
      if (!indexedDB) {
        return [];
      ...
      const adfFileStubs = await fileStorage.getLeafGenTechFileStubs();

      // For now, only regular files - drafts will be handled separately in the future
      const sortedFiles = adfFileStubs.sort((a, b) => (b.lastModified || 0) - (a.lastModified || 0));
  ...
  const handleRemoveFile = useCallback(async (index: number, files: GenTechFileStub[], setFiles: (files: GenTechFileStub[]) => void) => {
  ...
  const createFileSelectionHandlers = useCallback((
    ...
    const selectMultipleFiles = async (files: GenTechFileStub[], onGenTechFilesSelect: (adfFiles: GenTechFile[]) => void) => {
      if (selectedFiles.length === 0) return;

      try {
        // Filter by UUID and load full GenTechFile objects directly
        const selectedFileObjects = files.filter(f => selectedFiles.includes(f.id));

        const adfFiles = await Promise.all(
