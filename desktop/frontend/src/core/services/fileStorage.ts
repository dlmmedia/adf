...
import { GenTechFile, GenTechFileStub, createGenTechFile } from '@app/types/fileContext';
import { indexedDBManager, DATABASE_CONFIGS } from '@app/services/indexedDBManager';

/**
 * Storage record - single source of truth
 * Contains all data needed for both GenTechFile and GenTechFileStub
 */
export interface StoredGenTechFileRecord extends BaseFileMetadata {
  data: ArrayBuffer;
  fileId: FileId; // Matches runtime GenTechFile.fileId exactly
  quickKey: string; // Matches runtime GenTechFile.quickKey exactly
  thumbnail?: string;
...
class FileStorageService {
  ...
  /**
   * Store a GenTechFile with its metadata from GenTechFileStub
   */
  async storeGenTechFile(adfFile: GenTechFile, stub: GenTechFileStub): Promise<void> {
    const db = await this.getDatabase();
    const arrayBuffer = await adfFile.arrayBuffer();

    const record: StoredGenTechFileRecord = {
      id: adfFile.fileId,
      fileId: adfFile.fileId, // Explicit field for clarity
      quickKey: adfFile.quickKey,
      name: adfFile.name,
      type: adfFile.type,
      size: adfFile.size,
      lastModified: adfFile.lastModified,
      data: arrayBuffer,
      ...
      originalFileId: stub.originalFileId ?? adfFile.fileId,
  ...
  /**
   * Get GenTechFile with full data - for loading into workbench
   */
  async getGenTechFile(id: FileId): Promise<GenTechFile | null> {
    const db = await this.getDatabase();
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        const record = request.result as StoredGenTechFileRecord | undefined;
        if (!record) {
          resolve(null);
          return;
        }
        ...
        // Convert to GenTechFile with preserved IDs
        const adfFile = createGenTechFile(file, record.fileId);
        resolve(adfFile);
      };
    });
  }
  ...
  /**
   * Get multiple GenTechFiles - for batch loading
   */
  async getGenTechFiles(ids: FileId[]): Promise<GenTechFile[]> {
    const results = await Promise.all(ids.map(id => this.getGenTechFile(id)));
    return results.filter((file): file is GenTechFile => file !== null);
  }

  /**
   * Get GenTechFileStub (metadata only) - for UI browsing
   */
  async getGenTechFileStub(id: FileId): Promise<GenTechFileStub | null> {
    const db = await this.getDatabase();
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        // Create GenTechFileStub from metadata (no file data)
        const stub: GenTechFileStub = {
          id: record.id,
          name: record.name,
          type: record.type,
          size: record.size,
  ...
  /**
   * Get all GenTechFileStubs (metadata only) - for FileManager browsing
   */
  async getAllGenTechFileStubs(): Promise<GenTechFileStub[]> {
    const db = await this.getDatabase();
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        if (cursor) {
          const record = cursor.value as StoredGenTechFileRecord;
  ...
  /**
   * Get leaf GenTechFileStubs only - for unprocessed files
   */
  async getLeafGenTechFileStubs(): Promise<GenTechFileStub[]> {
    const db = await this.getDatabase();
    ...
    return new Promise((resolve, reject) => {
      ...
      const store = transaction.objectStore(this.storeName);
      const request = store.openCursor();
      const leafStubs: GenTechFileStub[] = [];
  ...
  /**
   * Delete GenTechFile - single operation, no sync issues
   */
  async deleteGenTechFile(id: FileId): Promise<void> {
    const db = await this.getDatabase();
    ...
    return new Promise((resolve, _reject) => {
      try {
        ...
        getRequest.onsuccess = () => {
          const record = getRequest.result as StoredGenTechFileRecord;
          if (record) {
  ...
  async getStorageStats(): Promise<StorageStats> {
    ...
    try {
      ...
      const stubs = await this.getAllGenTechFileStubs();
  ...
  async markFileAsProcessed(fileId: FileId): Promise<boolean> {
    try {
      ...
      const record = await new Promise<StoredGenTechFileRecord | undefined>((resolve, reject) => {
