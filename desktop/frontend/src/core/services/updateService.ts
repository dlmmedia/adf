...
export class UpdateService {
  private readonly baseUrl = 'https://dlmworld.com/api/updates';
  ...
  getDownloadUrl(machineInfo: MachineInfo): string | null {
    ...
    if (machineInfo.machineType === 'Server-jar') {
      return DOWNLOAD_BASE_URL + (machineInfo.activeSecurity ? 'DLM ADF-with-login.jar' : 'DLM ADF.jar');
    }
  ...
  /**
   * Get current version from GitHub releases as fallback
   */
  async getCurrentVersionFromGitHub(): Promise<string> {
    const url = 'https://api.github.com/repos/dlmmedia/adf/releases/latest';
