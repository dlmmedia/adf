...
import { GenTechFileStub, createGenTechFile } from '@app/types/fileContext';
...
export class ZipFileService {
      ...
      for (let i = 0; i < pdfFiles.length; i++) {
        ...
        try {
          ...
          if (await this.isValidPdfFile(extractedFile)) {
            ...
            result.errors.push(`File "${filename}" is not a valid ADF`);
          }
        } catch (error) {
  ...
  /**
   * Check if a GenTechFileStub represents a ZIP file (for UI checks without loading full file)
   */
  public isZipFileStub(stub: GenTechFileStub): boolean {
   ...
   * @param zipStub - The GenTechFileStub for the ZIP (contains metadata to preserve)
   * @returns Object with success status, extracted stubs, and any errors
   */
  async extractAndStoreFilesWithHistory(
    zipFile: File,
    zipStub: GenTechFileStub
  ): Promise<{ success: boolean; extractedStubs: GenTechFileStub[]; errors: string[] }> {
    const result = {
      success: false,
      extractedStubs: [] as GenTechFileStub[],
      errors: [] as string[]
    };
    ...
    try {
      ...
      for (const extractedFile of extractionResult.extractedFiles) {
        try {
          ...
          // Create GenTechFile
          const newGenTechFile = createGenTechFile(extractedFile);

          // Create GenTechFileStub with ZIP's history metadata
          const stub: GenTechFileStub = {
            id: newGenTechFile.fileId,
            name: extractedFile.name,
            size: extractedFile.size,
            type: extractedFile.type,
            lastModified: extractedFile.lastModified,
            quickKey: newGenTechFile.quickKey,
            createdAt: Date.now(),
            isLeaf: true,
          ...
          // Store in IndexedDB
          await fileStorage.storeGenTechFile(newGenTechFile, stub);
