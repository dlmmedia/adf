...
const Compare = (props: BaseToolProps) => {
  ...
  const prepareWorkbenchForRun = useCallback(
    (
      baseId: FileId | null,
      compId: FileId | null,
      options?: { baseFile?: GenTechFile | null; comparisonFile?: GenTechFile | null }
    ) => {
      if (!baseId || !compId) {
        return;
      }
  ...
  const renderSelectedFile = useCallback(
    (role: 'base' | 'comparison') => {
      ...
      const stub = fileId ? selectors.getGenTechFileStub(fileId) : undefined;
  ...
  const baseStub = params.baseFileId ? selectors.getGenTechFileStub(params.baseFileId) : undefined;
  const compStub = params.comparisonFileId ? selectors.getGenTechFileStub(params.comparisonFileId) : undefined;
