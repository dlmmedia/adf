...
const PdfTextEditor = ({ onComplete, onError }: BaseToolProps) => {
  ...
  const handleSaveToWorkbench = useCallback(async () => {
    ...
    try {
      ...
      const parentStub = selectors.getGenTechFileStub(sourceFileIdRef.current as any);
      if (!parentStub) {
      ...
      // Create GenTechFile and stub for the output
      const { adfFiles, stubs } = await createGenTechFilesAndStubs(
        [pdfFile],
        parentStub,
        'pdfTextEditor',
      );
      ...
      await consumeFiles([sourceFileIdRef.current as any], adfFiles, stubs);
