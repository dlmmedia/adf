...
/**
 * GenTechFileStub - Metadata record for files in the active workbench session
 *
 ...
 */
/**
 * GenTechFileStub - Runtime UI metadata for files in the active workbench session
 *
 * Contains UI display data and processing state. Actual File objects stored
 * separately in refs for memory efficiency. Supports multi-tool workflows
 * where files persist across tool operations.
 */
export interface GenTechFileStub extends BaseFileMetadata {
...
export interface FileContextNormalizedFiles {
  ids: FileId[];
  byId: Record<FileId, GenTechFileStub>;
}
...
// DLM ADF file with embedded UUID - replaces loose File + FileId parameter passing
export interface GenTechFile extends File {
...
export function isGenTechFile(file: File | Blob): file is GenTechFile {
...
export function getFormFillFileId(file: File | Blob | null | undefined): string | null {
  ...
  if (isGenTechFile(file)) {
    return `dlm-adf-${file.fileId}`;
  }
...
// Create a GenTechFile from a regular File object
export function createGenTechFile(file: File, id?: FileId): GenTechFile {
  // If the file already has GenTech metadata and we aren't trying to override it,
  ...
  return file as GenTechFile;
}

// Extract FileIds from GenTechFile array
export function extractFileIds(files: GenTechFile[]): FileId[] {
  return files.map(file => file.fileId);
}

// Extract regular File objects from GenTechFile array
export function extractFiles(files: GenTechFile[]): File[] {
  return files as File[];
}

// Check if an object is a File or GenTechFile (replaces instanceof File checks)
export function isFileObject(obj: any): obj is File | GenTechFile {
  return obj &&
...
export function createNewGenTechFileStub(
  file: File,
  id?: FileId,
  thumbnail?: string,
...
export function revokeFileResources(record: GenTechFileStub): void {
...
export type FileContextAction =
  ...
  | { type: 'ADD_FILES'; payload: { adfFileStubs: GenTechFileStub[] } }
  | { type: 'REMOVE_FILES'; payload: { fileIds: FileId[] } }
  | { type: 'UPDATE_FILE_RECORD'; payload: { id: FileId; updates: Partial<GenTechFileStub> } }
  ...
  | { type: 'CONSUME_FILES'; payload: { inputFileIds: FileId[]; outputGenTechFileStubs: GenTechFileStub[] } }
  | { type: 'UNDO_CONSUME_FILES'; payload: { inputGenTechFileStubs: GenTechFileStub[]; outputFileIds: FileId[] } }
...
export interface FileContextActions {
  ...
  addFiles: (files: File[], options?: { insertAfterPageId?: string; selectFiles?: boolean }) => Promise<GenTechFile[]>;
  addGenTechFileStubs: (adfFileStubs: GenTechFileStub[], options?: { insertAfterPageId?: string; selectFiles?: boolean }) => Promise<GenTechFile[]>;
  removeFiles: (fileIds: FileId[], deleteFromStorage?: boolean) => Promise<void>;
  updateGenTechFileStub: (id: FileId, updates: Partial<GenTechFileStub>) => void;
  reorderFiles: (orderedFileIds: FileId[]) => void;
  clearAllFiles: () => Promise<void>;
  clearAllData: () => Promise<void>;

  // File pinning - accepts GenTechFile for safer type checking
  pinFile: (file: GenTechFile) => void;
  unpinFile: (file: GenTechFile) => void;

  // File consumption (replace unpinned files with outputs)
  consumeFiles: (inputFileIds: FileId[], outputGenTechFiles: GenTechFile[], outputGenTechFileStubs: GenTechFileStub[]) => Promise<FileId[]>;
  undoConsumeFiles: (inputFiles: File[], inputGenTechFileStubs: GenTechFileStub[], outputFileIds: FileId[]) => Promise<void>;
...
export interface FileContextSelectors {
  getFile: (id: FileId) => GenTechFile | undefined;
  getFiles: (ids?: FileId[]) => GenTechFile[];
  getGenTechFileStub: (id: FileId) => GenTechFileStub | undefined;
  getGenTechFileStubs: (ids?: FileId[]) => GenTechFileStub[];
  getAllFileIds: () => FileId[];
  getSelectedFiles: () => GenTechFile[];
  getSelectedGenTechFileStubs: () => GenTechFileStub[];
  getPinnedFileIds: () => FileId[];
  getPinnedFiles: () => GenTechFile[];
  getPinnedGenTechFileStubs: () => GenTechFileStub[];
  isFilePinned: (file: GenTechFile) => boolean;
