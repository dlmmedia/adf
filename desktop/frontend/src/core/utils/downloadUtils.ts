 ...
 */
export async function downloadFileFromStorage(file: GenTechFileStub): Promise<void> {
  const lookupKey = file.id;
  const adfFile = await fileStorage.getGenTechFile(lookupKey);
 ...
 */
export async function downloadMultipleFiles(files: GenTechFileStub[]): Promise<void> {
 ...
 */
export async function downloadFilesAsZip(files: GenTechFileStub[], zipFilename?: string): Promise<void> {
  if (files.length === 0) {
  ...
  for (const fileWithUrl of files) {
    ...
    if (adfFile) {
      // GenTechFile is already a File object!
      filesToZip.push(adfFile);
    }
  }
