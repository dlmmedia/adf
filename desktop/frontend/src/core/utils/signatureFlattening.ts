...
import {createGenTechFile, FileId, GenTechFile, GenTechFileStub} from '@app/types/fileContext';
...
export interface SignatureFlatteningResult {
  inputFileIds: FileId[];
  outputGenTechFile: GenTechFile;
  outputStub: GenTechFileStub;
}
    ...
    if (pdfArrayBuffer) {
      ...
      if (!currentFile) {
        ...
        if (allFileIds.length > 0) {
          ...
          const fileStub = selectors.getGenTechFileStub(allFileIds[fileIndex]);
          const fileObject = selectors.getFile(allFileIds[fileIndex]);
          if (fileStub && fileObject) {
            currentFile = createGenTechFile(fileObject, allFileIds[fileIndex] as FileId);
          }
        }
      }
      ...
      const record = selectors.getGenTechFileStub(currentFile.fileId);
      if (!record) {
      ...
      const outputGenTechFile = createGenTechFile(signedFile, outputStub.id);
