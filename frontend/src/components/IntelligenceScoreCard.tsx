  ...
  const metrics = [
    {
      ...
      value: benchmarks.structure_accuracy,
      color: "#3b82f6",
    },
    {
      label: "Entity Recognition",
      value: benchmarks.entity_accuracy,
      color: "#8b5cf6",
    },
    {
      label: "AI Readiness",
      value: Math.min(0.99, benchmarks.structure_accuracy * 0.5 + benchmarks.entity_accuracy * 0.5 + 0.1),
      color: "#06b6d4",
    },
    {
      label: "Token Savings",
      value: benchmarks.token_savings_percent / 100,
      color: "#10b981",
    },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {metrics.map((m, i) => (
          <GaugeRow key={m.label} {...m} delay={i * 0.15} />
        ))}
      </div>
      <div className="mt-4 pt-4 border-t border-white/5">
        <div className="flex justify-between text-sm">
          ...
          <span className="text-white font-medium">
            {(benchmarks.conversion_time_ms / 1000).toFixed(1)}s
          </span>
        </div>
...
export function BenchmarkComparison({ benchmarks }: { benchmarks: BenchmarkData }) {
  ...
  useEffect(() => {
    ...
    const data = [
      { label: "PDF Parsing", value: pdfTime, color: "#ef4444" },
